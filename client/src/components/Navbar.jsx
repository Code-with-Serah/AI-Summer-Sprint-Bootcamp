...
const Navbar = () => {
  ...
  return (
    <div className='fixed top-0 left-0 z-50 w-full flex items-center justify-between px-6 md:px-36 py-5'>
        ...
        <img src={assets.logo} alt='dada' className='w-36 h-auto'/>
        </Link>
        ...
        border-gray-300/20 overflow-hidden transition-[width] duration-300 ${isOpen ? 'max-md:w-full':'max-md:w-0'}`}>
          ...
          <XIcon className='md:hidden absolute top-6 right-6 w-6 h-6 cursor-pointer' onClick={()=>setIsOpen(!isOpen)}/>
           <Link onClick={()=>{scrollTo(0,0);setIsOpen(false)}} to='/'>Home</Link>
            <Link onClick={()=>{scrollTo(0,0);setIsOpen(false)}} to='/MoviesList'>Movies</Link>
        ...
        <MenuIcon onClick={()=>{setIsOpen(!isOpen)}} className='max-md:ml-4 md:hidden w-8 cursor-pointer' />
    </div>
  )
}
